...
export function renderSimulador() {
    ...
    renderTutorial('ft-sec-sim', 'sim', ico.simulator, 'Como usar o Simulador', [
        ...
        '<strong>Overhead</strong>: % sobre o custo de ingredientes (gás, embalagem, energia).',
        '<strong>Mão de obra</strong>: valor fixo em R$ adicionado por pizza.',
        '<strong>Comparar</strong>: veja até 4 pizzas lado a lado.',
    ]);
...
function _renderComparar(recs) {
    ...
    const opts = recs.map(r => `<option value="${r.id}">${esc(r.nome)} (${r.tamanho})</option>`).join('');
    return `
...
function _renderComparacaoLive() {
    ...
    document.getElementById('ft-cmp-btn')?.addEventListener('click', () => {
        ...
        const ovh = _cfg.overhead, mdo = _cfg.maoDeObra;
